...
bool RendererHw::render(
	...
	glBufferDataARB(GL_ARRAY_BUFFER_ARB, rndModel->vertices.size() * sizeof(float), rndModel->vertices.data(), GL_STATIC_DRAW_ARB);
	...
	if (useMap) {
		...
		glBufferDataARB(GL_ARRAY_BUFFER_ARB, rndModel->uvcoords.size() * sizeof(float), rndModel->uvcoords.data(), GL_STATIC_DRAW_ARB);
	...
	if (useCpv) {
		...
		glBufferDataARB(GL_ARRAY_BUFFER_ARB, rndModel->colors.size() * sizeof(float), rndModel->colors.data(), GL_STATIC_DRAW_ARB);
	...
	if (useMap) {
		...
		std::cout << "Time on texture upload to GPU: " << ((float)(clock() - t1)) / CLOCKS_PER_SEC << " sec." << std::endl;
	}
	else {
	...
	if (useBBox) {
		...
		computeBBox(rndModel->vertices, minPos, maxPos);
	}
	...
	std::cout << "BBox = " << minPos.x << "," << minPos.y << "," << minPos.z << "/"
