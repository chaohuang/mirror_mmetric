...
const char* Render::brief = "Render a mesh (todo points)";

//
Command* Render::create() { return new Render(); }

// 
bool Render::initialize(Context* ctx, std::string app, int argc, char* argv[])
{
	...
	try
	{
		...
		//
		if (result.count("outputImage"))
			outputImageFilename = result["outputImage"].as<std::string>();
		//
		if (result.count("outputDepth"))
			outputDepthFilename = result["outputDepth"].as<std::string>();
		//
		if (result.count("renderer"))
			renderer = result["renderer"].as<std::string>();

		//
		if (result.count("width"))
			width = result["width"].as<unsigned int>();
		if (result.count("height"))
			height = result["height"].as<unsigned int>();
		...
		if (result.count("viewDir")) {
			...
			if (!parseVec3(viewDirStr, viewDir)) {
				std::cout << "Error: parsing --viewDir=\"" << viewDirStr << "\" expected three floats with space separator" << std::endl;
				return false;
			}
		}
		...
		if (result.count("viewUp")) {
			...
			if (!parseVec3(viewUpStr, viewUp)) {
				std::cout << "Error: parsing --viewUp=\"" << viewUpStr << "\" expected three floats with space separator" << std::endl;
				return false;
			}
		}
		...
		if (result.count("bboxMin") && result.count("bboxMax")) {
			...
			if (!parseVec3(bboxMinStr, bboxMin)) {
				std::cout << "Error: parsing --bboxMin=\"" << bboxMinStr << "\" expected three floats with space separator" << std::endl;
				return false;
			}
			if (!parseVec3(bboxMaxStr, bboxMax)) {
				std::cout << "Error: parsing --bboxMax=\"" << bboxMaxStr << "\" expected three floats with space separator" << std::endl;
				return false;
			}
			bboxValid = true;
		}
		...
		if (result.count("clearColor")) {
			...
			if (!parseVec4(clearColorStr, clearColor)) {
				std::cout << "Error: parsing --clearColor=\"" << clearColorStr << "\" expected four floats with space separator" << std::endl;
				return false;
			}
		}
		// lighting
		if (result.count("enableLighting"))
			enableLighting = result["enableLighting"].as<bool>();
		
		if (result.count("autoLightPosition"))
			autoLightPosition = result["autoLightPosition"].as<bool>();

		if (result.count("lightAutoDir")) {
			lightAutoDirStr = result["lightAutoDir"].as<std::string>();
			if (!parseVec3(lightAutoDirStr, lightAutoDir)) {
				std::cout << "Error: parsing --lightAutoDir=\"" << clearColorStr << "\" expected four floats with space separator" << std::endl;
				return false;
			}
		}

		// culling
		if (result.count("enableCulling"))
			enableCulling = result["enableCulling"].as<bool>();
		if (result.count("cwCulling"))
			cwCulling = result["cwCulling"].as<bool>();
	}
	...
	if (renderer == "gl12_raster") {
		if (!_hwRenderer.initialize(width, height)) {
			return false;
		}
	}
...
bool Render::process(uint32_t frame) {
	...
	if (renderer == "sw_raster")
	{
		...
		res = _swRenderer.render(
			inputModel, textureMap,
			outImage, outDepth,
			width, height, 
			viewDir, viewUp,
			bboxMin, bboxMax, bboxValid);
	}
