...
const char* Normals::name="normals";
const char* Normals::brief="Computes the mesh normals.";

//
Command* Normals::create() { return new Normals(); }

//
bool Normals::initialize(Context* ctx, std::string app, int argc, char* argv[])
{
	...
	try
	{
		...
		//
		if (result.count("normalized"))
			_normalized = result["normalized"].as<bool>();
		if (result.count("noSeams"))
			_noSeams = result["noSeams"].as<bool>();
	}
...
bool Normals::process(uint32_t frame) {
